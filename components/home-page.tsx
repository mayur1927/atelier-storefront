...
export function HomePage() {
  ...
  const subscribe = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmedEmail) {
      setNewsletterMessage("Enter your email address to subscribe.");
      return;
    }
    ...
    <>
      <section className="mx-auto max-w-7xl px-4 pt-4 sm:px-6 lg:px-8 lg:pt-8">
        <div className="relative min-h-[530px] overflow-hidden rounded-3xl bg-zinc-200 sm:min-h-[590px]">
          <img
            src="https://images.unsplash.com/photo-1485968579580-b6d095142e6e?auto=format&fit=crop&w=1800&q=85"
          ...
          <div className="relative flex min-h-[530px] max-w-xl flex-col justify-end p-7 text-white sm:min-h-[590px] sm:p-12 lg:p-16">
            ...
            <h1 className="max-w-lg text-4xl font-black leading-[0.98] tracking-[-0.065em] sm:text-6xl">
              New arrivals, made for slow summer days.
            </h1>

            <p className="mt-5 max-w-md text-sm leading-6 text-zinc-200 sm:text-base">
              A refined edit of lightweight layers, easy tailoring, and pieces that feel right from morning through late evening.
            </p>

            <Link
              href="#best-selling"
              className="mt-8 inline-flex w-fit items-center gap-3 rounded-full bg-white px-6 py-3.5 text-xs font-bold tracking-[0.13em] text-zinc-950 transition hover:bg-zinc-200"
            >
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="grid divide-y divide-zinc-200 rounded-2xl border border-zinc-200 bg-white sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
          {features.map(({ label, detail, icon: Icon }) => (
      ...
      <section
        id="best-selling"
        className="mx-auto max-w-7xl px-4 pb-4 pt-8 sm:px-6 lg:px-8"
      >
        <div className="mb-8 flex items-end justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-zinc-400">
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex flex-col items-start justify-between gap-6 rounded-3xl bg-zinc-950 px-7 py-10 text-white sm:flex-row sm:items-end sm:px-12 sm:py-12">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-zinc-400">
              The Atelier note
            </p>

            <h2 className="mt-3 max-w-xl text-3xl font-black tracking-[-0.05em] sm:text-4xl">
          ...
          {subscribed ? (
            <p
              role="status"
              className="w-full max-w-sm rounded-full border border-zinc-700 bg-zinc-900 px-5 py-3.5 text-center text-sm font-bold text-white"
            >
              You&apos;re subscribed!
            </p>
          ) : (
            ...
            >
              <div className="flex gap-2">
                <input
                  ...
                  aria-invalid={Boolean(newsletterMessage)}
                  aria-describedby="newsletter-message"
                  placeholder="Your email address"
                  className="min-w-0 flex-1 rounded-full bg-white px-4 py-3 text-sm text-zinc-950 outline-none ring-offset-2 focus:ring-2 focus:ring-white"
                />
              ...
              <p
                ...
                role="alert"
                aria-live="polite"
                className="mt-2 min-h-4 px-2 text-xs text-zinc-300"
              >
                {newsletterMessage}
              </p>
            </form>
